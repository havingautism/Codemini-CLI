...
export function StatusBar({ runtimeState, live, stageLabel }) {
  ...
  const contextColor = pct < 40 ? 'bg-(--accent-green)' : pct < 75 ? 'bg-(--accent-orange)' : 'bg-(--accent-red)';

  const stageKey = Object.keys(STAGE_COLORS).find(k => (stageLabel || '').includes(k)) || 'live';
      ...
      {max > 0 && (
        <span className="inline-flex items-center gap-1 whitespace-nowrap">
          ...
          <span className="w-12 h-1 bg-(--muted) rounded-full overflow-hidden">
            <span className={cn('block h-full rounded-full transition-all', contextColor)} style={{ width: `${pct}%` }} />
          </span>
          <span>{pct}%</span>
        </span>
      )}

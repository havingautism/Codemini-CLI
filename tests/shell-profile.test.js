...
test('shell system prompt reminds the model it is a CLI coding agent and to send a short progress update before tools', () => {
  ...
  assert.match(prompt, /path:"src\/app\.ts"/i);
  assert.match(prompt, /old_text:"foo", new_text:"bar"/i);
  assert.match(prompt, /path:"notes\.txt", content:"\.\.\."/i);
  assert.match(prompt, /prefer absolute path values rooted there/i);

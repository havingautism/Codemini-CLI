...
test('tower overlay keeps run_subagent even when the route prefers fork_task', async () => {
  ...
  assert.equal(isCodingRouteToolAllowed(result, 'crew_status'), false);
  assert.equal(isCodingRouteToolAllowed(result, 'crew_status', { towerActive: true }), true);
  assert.equal(isCodingRouteToolAllowed(result, 'cancel_worker'), false);
  assert.equal(isCodingRouteToolAllowed(result, 'cancel_worker', { towerActive: true }), true);

...
```text
codemini-cli-0.5.0.tgz
```

If you want to verify the package contents:

```bash
tar -tf codemini-cli-0.5.0.tgz
```
...
```powershell
npm install -g C:\temp\codemini-cli-0.5.0.tgz
```
...
cd C:\temp\coder-test
npm install C:\temp\codemini-cli-0.5.0.tgz
```
